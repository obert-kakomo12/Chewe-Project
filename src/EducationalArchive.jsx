...
const EducationalArchive = () => {
  ...
  const handleExport = () => {
    ...
    setTimeout(() => {
      const pdf = new jsPDF();
      pdf.text(`Master Educational Ledger - ${exportYear}`, 20, 20);
      pdf.text(`Generated by CT Tech's Pulse`, 20, 30);
      pdf.text(`Total Cohorts: ${filteredArchives.length}`, 20, 40);
      pdf.save(`Master_Ledger_${exportYear}.pdf`);
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        ...
                        <button className="icon-button" title="Export as PDF" style={{ color: 'var(--accent-blue)' }}
                          onClick={() => {
                            const pdf = new jsPDF();
                            pdf.text(`Archive Cohort: ${a.cohort}`, 20, 20);
                            pdf.text(`Size: ${a.size}`, 20, 30);
                            pdf.text(`Compression: ${a.compression}`, 20, 40);
                            pdf.text(`Location: ${a.location}`, 20, 50);
                            pdf.text(`Status: ${a.status}`, 20, 60);
                            pdf.save(`${a.cohort.replace(/\s+/g, '_')}_Archive.pdf`);
                          }}
                        >
                          <Download size={15} />
                        </button>
